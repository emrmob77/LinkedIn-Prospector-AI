...
- **AI Engine**: 4 provider desteği (Anthropic Claude, OpenAI GPT, Google Gemini, OpenRouter)
- **Data Collection**: Chrome Extension (Manifest V3) for LinkedIn data capture
- **Authentication**: Supabase Auth (email/password + Extension direkt login)
...
graph TB
    ...
    subgraph "Chrome Extension"
        ...
        ExtPopup[Popup UI]
        ExtBG[Background Service Worker]
    end
    
    subgraph "Service Layer"
        ImportService[Extension Import Service]
        AIEngine[AI Classification Service]
        DOMParser[LinkedIn DOM Parser]
    ...
    subgraph "External Services"
        ...
        AI[AI Providers - Claude/GPT/Gemini/OpenRouter]
    end
...
- `POST /api/extension/import`: Chrome Extension'dan post verisi alır
- `GET /api/extension/me`: Extension bağlantı durumu kontrolü
- `POST /api/posts/classify`: AI sınıflandırma + lead çıkarma + skorlama
- `POST /api/leads/extract`: Manuel lead çıkarma
...
- **Extension Import Service**: Chrome Extension'dan gelen post verilerini işler ve veritabanına kaydeder
- **AI Classification Service**: 4 provider destekli filtreleme, puanlama ve mesaj oluşturma (kullanıcı bazlı API key + dinamik prompt)
- **LinkedIn DOM Parser**: Extension'ın yakaladığı LinkedIn DOM verisinden yapılandırılmış veri çıkarır
...
- **Supabase PostgreSQL**: Çekirdek tablolar (search_runs, posts, leads, lead_posts, messages, activity_logs, user_settings, saved_searches)
...
1. Kullanıcı LinkedIn'de manuel arama yapar (kendi oturumu ile kişiselleştirilmiş sonuçlar)
2. Chrome Extension sayfadaki postları otomatik algılar ve DOM'dan parse eder
3. Kullanıcı Extension popup'ından "İçe Aktar" butonuna tıklar
4. Extension, post verilerini `POST /api/extension/import` endpoint'ine gönderir
5. API `search_runs` kaydı oluşturur (source: 'chrome_extension')
6. Import Service, verileri doğrular ve `posts` tablosunda saklar
7. Search sayfası açıldığında sınıflandırılmamış post varsa otomatik AI sınıflandırma başlar
8. AI Engine ilgiliği sınıflandırır, lead çıkarır ve puanlar
9. Potansiyel müşteriler yineleme kontrolü ile `leads` tablosunda saklanır
10. Aktivite süreç boyunca kaydedilir

**Desteklenen LinkedIn Sayfa Tipleri:**
...
3. AI Engine, kullanıcının tercih ettiği AI provider'ı kullanarak kişiselleştirilmiş mesaj oluşturur
...
```typescript
interface ExtensionImportService {
  /**
   * Chrome Extension'dan gelen post verilerini işler
   * @param posts - Extension'ın DOM'dan parse ettiği post dizisi
   * @param source - Kaynak sayfa tipi (search, company, profile, feed)
   * @returns İşlenmiş ve kaydedilmiş post sonuçları
   */
  importPosts(
    posts: ExtensionPostData[],
    source: 'search' | 'company_page' | 'profile' | 'feed'
  ): Promise<ImportResult>;

  /**
   * Extension'dan gelen veriyi doğrular
   * @param data - Ham extension verisi
   * @returns Doğrulama sonucu
   */
  validateExtensionData(data: unknown): ValidationResult;
}
...
#### LinkedIn DOM Parser (Chrome Extension içinde)

```typescript
interface LinkedInDOMParser {
  /**
   * LinkedIn sayfasındaki post kartlarını DOM'dan parse eder
   * @returns Parse edilmiş post verisi dizisi
   */
  parsePostCards(): ExtensionPostData[];

  /**
   * Sayfa tipini URL'ye göre algılar
   * @param url - Mevcut sayfa URL'si
   * @returns Sayfa tipi
   */
  detectPageType(url: string): 'search' | 'company_page' | 'profile' | 'feed' | 'unknown';

  /**
   * Sayfadaki toplam post sayısını döndürür
   */
  getPostCount(): number;
}
